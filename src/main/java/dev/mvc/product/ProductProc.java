...
public class ProductProc implements ProductProcInter {
  ...
  public String pagingBox(String listFile, int productcateno, int search_count, int nowPage, String word){ 
     int totalPage = (int)(Math.ceil((double)search_count/Products.RECORD_PER_PAGE)); // 전체 페이지  
    int totalGrp = (int)(Math.ceil((double)totalPage/Products.PAGE_PER_BLOCK));// 전체 그룹 
    int nowGrp = (int)(Math.ceil((double)nowPage/Products.PAGE_PER_BLOCK));    // 현재 그룹 
    int startPage = ((nowGrp - 1) * Products.PAGE_PER_BLOCK) + 1; // 특정 그룹의 페이지 목록 시작  
    int endPage = (nowGrp * Products.PAGE_PER_BLOCK);             // 특정 그룹의 페이지 목록 종료   
...
//    str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
    ...
    int _nowPage = (nowGrp-1) * Products.PAGE_PER_BLOCK;  
    if (nowGrp >= 2){ 
      str.append("<span class='span_box_1'><A href='"+listFile+"?&word="+word+"&nowPage="+_nowPage+"&productcateno="+productcateno+"'>이전</A></span>"); 
    } 
    ...
    _nowPage = (nowGrp * Products.PAGE_PER_BLOCK)+1;  
    if (nowGrp < totalGrp){ 
      str.append("<span class='span_box_1'><A href='"+listFile+"?&word="+word+"&nowPage="+_nowPage+"&productcateno="+productcateno+"'>다음</A></span>"); 
    } 
    str.append("</DIV>"); 
